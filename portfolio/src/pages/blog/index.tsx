  ...
  return (
    <>
      <main className="flex flex-col min-h-[85vh] p-4 mx-auto max-w-[600px]">
        ...
        <div className="w-full bg-black border-black rounded-md drop-shadow-3xl subpixel-antialiased">
          ...
          <div className="pl-1 pt-1 h-auto text-green-200 font-mono text-xs bg-black rounded-md">
            <div className="font-bold text-green-300 pb-1">{`> cat blogPosts.txt`}</div>

            {allPostsData.map(({ id, date, title }) => (
              <div key={id} className="mb-4">
                <div className="pl-4 pb-1">
                  <Link href={`/blog/${id}`}>
                    <p className="font-bold text-white hover:underline ml-4">
                      {date + " " + title}
                    </p>
                  </Link>
                </div>
